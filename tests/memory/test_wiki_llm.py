...
@pytest.fixture
async def wiki_llm(reasoning_llm, query_index):
    global _wiki_llm

    if _wiki_llm is None:
        embedding_db = EmbeddingDatabase(reasoning_llm, query_index)
        wiki_module = WikiModule(reasoning_llm, embedding_db)
        await wiki_module.add_file(
            "/story/chapter_1.txt", Value("story text", STORY_PART_1)
        )
        await wiki_module.add_file(
            "/story/chapter_2.txt", Value("story text", STORY_PART_2)
        )
        _wiki_llm = WikiLLMEngine(reasoning_llm, wiki_module)

    return _wiki_llm


@pytest.mark.asyncio
async def test_data_loaded(wiki_llm: WikiLLMEngine):
    assert len(wiki_llm.wiki_module.files) == 2


@pytest.mark.asyncio
async def test_query_block(wiki_llm):
    game_task_response = await wiki_llm.query_block(
        "text",
...
@pytest.mark.asyncio
async def test_query_object(wiki_llm):
    class Response(BaseModel):
        author: str

    response = await wiki_llm.query_object(
        Response,
...
@pytest.mark.asyncio
async def test_load_fsbank_llm(reasoning_llm, wiki_llm):
    state_dict = wiki_llm.state_dict()
    restored = WikiLLMEngine.load_state_dict(state_dict)

    assert isinstance(restored, WikiLLMEngine)
    assert isinstance(restored.underlying_llm, type(reasoning_llm))
    assert isinstance(restored.wiki_module, WikiModule)

    assert len(restored.wiki_module.files) == len(wiki_llm.wiki_module.files)
