...
from horsona.memory.fs_bank_llm import FilesystemBankLLMEngine
...
@pytest.fixture
async def fsbank_llm(reasoning_llm, query_index):
    global _fsbank_llm

    if _fsbank_llm is None:
...
@pytest.mark.asyncio
async def test_query_object(fsbank_llm):
...
@pytest.mark.asyncio
async def test_load_fsbank_llm(reasoning_llm, fsbank_llm):
    state_dict = fsbank_llm.state_dict()
    restored = FilesystemBankLLMEngine.load_state_dict(state_dict)
